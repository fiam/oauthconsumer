...
// When a call finishes, notify the delegate
- (void)callFinished:(OACall *)call body:(NSString *)body
{
...
- (void)requestToken {
	...
	if (atoken && [atoken isValid] && ![atoken isEqualToToken:acToken]) {
...
- (void)requestTokenReceived:(OACall *)call body:(NSString *)body
{
	...
	if (token) {
		...
		if ([delegate respondsToSelector:@selector(tokenManagerNeedsAuthorizedToken:fromURL:)]) {
			[delegate tokenManagerNeedsAuthorizedToken:self fromURL:url];
		} else {
...
- (void)fetchData:(NSString *)aURL method:(NSString *)aMethod parameters:(NSArray *)theParameters
			files:(NSArray *)theFiles finished:(SEL)didFinish delegate:(NSObject*)aDelegate {
...
- (void)fetchData:(NSString *)aURL method:(NSString *)aMethod parameters:(NSArray *)theParameters
			files:(NSArray *)theFiles finished:(SEL)didFinish {
...
- (void)fetchData:(NSString *)aURL parameters:(NSArray *)theParameters files:(NSArray *)theFiles
		 finished:(SEL)didFinish {
